 ...
 */    
    public function GetSubcription($id){
        ...
        $query ="SELECT  a.*, b.TownID, b.CountyID, b.CountryID, b.WebsiteAddress, b.TelephoneNumber FROM subscriptions AS a  LEFT JOIN organisations AS b  ON b.organisationID =  a.organisationID  where a.organisationID = $id" ;
        $result = phpmkr_query($query);
        if ($result){ 
            $row = $result->fetch_assoc();
            $row['success'] = true;
